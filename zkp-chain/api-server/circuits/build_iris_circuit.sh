#!/bin/bash
# ============================================================
# build_iris_circuit.sh — Compile Iris Circom circuit + Groth16 setup
...
CIRCUIT=iris_auth
PTAU_SIZE=18   # 2^18 = 262144 constraints (reuse same ptau as face circuit)
PTAU_FILE="powersOfTau28_hez_final_${PTAU_SIZE}.ptau"
PTAU_URL="https://storage.googleapis.com/zkevm/ptau/${PTAU_FILE}"

echo "============================================"
echo "  ZK-SNARK Iris Circuit Build (Groth16)"
echo "============================================"
...
# ------------------------------------------
# Step 3: Groth16 Setup (phase 2 contribution)
# ------------------------------------------
echo ""
...
npx snarkjs zkey contribute \
    ${CIRCUIT}_0000.zkey \
    ${CIRCUIT}.zkey \
    --name="Development Phase 2 (iris)" -v -e="iris entropy random text"
...
npx snarkjs zkey export verificationkey \
    ${CIRCUIT}.zkey \
    iris_verification_key.json

echo "  ✓ Verification key: iris_verification_key.json"
...
npx snarkjs zkey export solidityverifier \
    ${CIRCUIT}.zkey \
    IrisVerifier.sol 2>/dev/null || echo "  (skipped — not needed for off-chain)"

echo ""
echo "============================================"
echo "  Iris Circuit Build complete!"
echo ""
echo "  Files produced:"
echo "    ${CIRCUIT}.r1cs              — constraint system"
echo "    ${CIRCUIT}_js/               — WASM witness generator"
echo "    ${CIRCUIT}_cpp/              — C++ witness generator"
echo "    ${CIRCUIT}.zkey              — Groth16 proving key"
echo "    iris_verification_key.json   — verification key"
echo "============================================"
